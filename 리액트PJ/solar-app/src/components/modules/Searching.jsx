...
import { earthData, moonData, saturnData, jupiterData } from "../data/searching";
...
function Searching({ kword }) {
  ...
  // [3] 체크박스 체크여부 상태관리변수
  const [chk, setChk] = useState([true, true, true, true]);
 // 데이터 모으기
 const selData = [
  ...earthData,
  ...moonData,
...
// console.log("데이터가 selData에 잘 모아졌나?",selData);
  ...
  const newList = selData.filter((v) => {
    ...
    let newVal = v.cdesc.toLocaleLowerCase();
    // // 전달받은 키워드도 소문자처리
    // // ((중요!!!)) 상태변수인 kw로 대체한다!!!
    ...
    if (
      ...
      ((chk[0] ? v.alignment == "earth" : false) ||
        (chk[1] ? v.alignment == "moon" : false) ||
        (chk[2] ? v.alignment == "saturn" : false) ||
        (chk[3] ? v.alignment == "jupiter" : false)) 
  ...
  if (sort == "asc") {
    newList.sort((a, b) =>
      a.cdesc > b.cdesc ? 1 : a.cdesc < b.cdesc ? -1 : 0
    );
  } /// if ///////////////////////
  // (2) 내림차순일 경우
  else if (sort == "desc") {
